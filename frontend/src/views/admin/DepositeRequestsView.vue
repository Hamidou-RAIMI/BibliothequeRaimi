...
const filteredDepositeRequests = computed(() => {
  return depositeRequestStore.depositeRequests.filter(demande => {
    ...
    const matchesStatus = filterStatus.value === '' || demande.status === filterStatus.value
...
onMounted(async () => {
  try {
    await Promise.all([
      depositeRequestStore.fetchDepositeRequests(),
      depositeRequestStore.fetchManagers(),
...
const handleAssign = async () => {
  try {
    await depositeRequestStore.assignRequest(depositeRequestStore.currentDepositeRequest.id, selectedManagerId.value)
    toast.add({ severity: 'success', summary: 'Succès', detail: 'Demande affectée avec succès', life: 3000 })
    showAssignModal.value = false
  } catch (err) {
    console.error(err)
    toast.add({ severity: 'error', summary: 'Erreur', detail: 'Erreur lors de l\'affectation', life: 3000 })
  }
}

const handleReassign = async () => {
  try {
    await depositeRequestStore.reassignRequest(depositeRequestStore.currentDepositeRequest.id, selectedManagerId.value)
    toast.add({ severity: 'success', summary: 'Succès', detail: 'Demande réaffectée avec succès', life: 3000 })
    showAssignModal.value = false
  } catch (err) {
    console.error(err)
    toast.add({ severity: 'error', summary: 'Erreur', detail: 'Erreur lors de la réaffectation', life: 3000 })
  }
}
...
const handleReject = async () => {
  try {
    await depositeRequestStore.rejectRequest(depositeRequestStore.currentDepositeRequest.id, rejectJustification.value)
    toast.add({ severity: 'success', summary: 'Succès', detail: 'Demande rejetée avec succès', life: 3000 })
...
const handlePublish = async () => {
  try {
    await depositeRequestStore.publishRequest(depositeRequestStore.currentDepositeRequest.id)
    toast.add({ severity: 'success', summary: 'Succès', detail: 'Demande publiée avec succès', life: 3000 })
  } catch (err) {
    console.error(err)
    toast.add({ severity: 'error', summary: 'Erreur', detail: 'Erreur lors de la publication', life: 3000 })
  }
}
</script>
...
<template>
  <div class="p-6 max-w-8xl mx-auto">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          >
            ...
            <option value="reassigned">Réaffectée</option>
            <option value="approved_by_manager">Approuvée par responsable</option>
            <option value="rejected_by_manager">Rejetée par responsable</option>
        ...
        <table class="w-full">
          ...
          <tbody class="divide-y divide-gray-200">
            ...
            <tr v-else v-for="demande in filteredDepositeRequests" :key="demande.id" class="hover:bg-gray-50 transition">
              ...
              <td class="px-6 py-4 text-gray-600">
                {{ demande.assignedManager ? `${demande.assignedManager.first_name} ${demande.assignedManager.last_name}` : '-' }}
              </td>
              <td class="px-6 py-4">
        ...
        <div v-if="depositeRequestStore.currentDepositeRequest" class="space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div v-if="depositeRequestStore.currentDepositeRequest.subtitle">
              <label class="block text-sm font-medium text-gray-500">Sous-titre</label>
            ...
            <div class="md:col-span-2" v-if="depositeRequestStore.currentDepositeRequest.abstract">
              <label class="block text-sm font-medium text-gray-500">Résumé</label>
            ...
            <div>
              <label class="block text-sm font-medium text-gray-500">Année de publication</label>
            ...
            <div>
              <label class="block text-sm font-medium text-gray-500">Éditeur</label>
          ...
          <div class="flex flex-wrap gap-3 mt-6">
            <button
              v-if="depositeRequestStore.currentDepositeRequest.status === 'pending' ||
                     depositeRequestStore.currentDepositeRequest.status === 'rejected_by_manager'"
            ...
            <button
              v-if="depositeRequestStore.currentDepositeRequest.status === 'assigned' ||
                     depositeRequestStore.currentDepositeRequest.status === 'reassigned'"
              @click="openAssignModal"
            ...
            <button
              v-if="depositeRequestStore.currentDepositeRequest.status === 'approved_by_manager'"
              @click="handlePublish"
              :disabled="depositeRequestStore.loading"
              class="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition disabled:opacity-50"
            >
              Publier
            </button>
            <button
              v-if="depositeRequestStore.currentDepositeRequest.status !== 'published' &&
                     depositeRequestStore.currentDepositeRequest.status !== 'rejected'"
    ...
    <div v-if="showAssignModal" class="fixed inset-0 z-50 flex items-center justify-center">
      <div @click="showAssignModal = false" class="absolute inset-0 bg-black/50 backdrop-blur-sm"></div>
      <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 p-8">
        <h2 class="text-2xl font-bold text-gray-800 mb-6">
          {{ depositeRequestStore.currentDepositeRequest?.status === 'assigned' ||
             depositeRequestStore.currentDepositeRequest?.status === 'reassigned' ? 'Réaffecter la demande' : 'Affecter la demande' }}
        </h2>
        <div class="space-y-4">
          <div>
            ...
            >
              ...
              <option
                v-for="manager in depositeRequestStore.managers"
                :key="manager.id"
                :value="manager.id"
              >
          ...
          <button
            @click="
              ...
              depositeRequestStore.currentDepositeRequest?.status === 'reassigned'
                ...
                : handleAssign()
            "
            :disabled="!selectedManagerId || depositeRequestStore.loading"
            class="flex-1 px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white rounded-lg transition disabled:opacity-50"
          >
            {{ depositeRequestStore.currentDepositeRequest?.status === 'assigned' ||
               depositeRequestStore.currentDepositeRequest?.status === 'reassigned' ? 'Réaffecter' : 'Affecter' }}
          </button>
        </div>
      </div>
    </div>
    ...
    <div v-if="showRejectModal" class="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 p-8">
        <h2 class="text-2xl font-bold text-gray-800 mb-6">Rejeter la demande</h2>
        <div class="space-y-4">
          <div>
        ...
        <div class="flex gap-4 mt-6">
          ...
          <button
            @click="handleReject"
            :disabled="!rejectJustification || depositeRequestStore.loading"
